#!/usr/bin/env bash
# Build tb-active-receiver.xpi / tb-active-receiver-<version>.xpi from tb-active-receiver/ directory.
# See scripts/build-xpi.sh for the exact release/update rules.
...
MANIFEST_PATH="tb-active-receiver/manifest.json"
GECKO_ID="tb_active_receiver_standalone@markbai.thunderbird.local"
XPI_BASE="tb-active-receiver"
XPI_DIR="tb-active-receiver"
...
if [[ "$PUBLISH_MODE" == "2" ]]; then
  ...
  if [[ -n "$(git status --porcelain "${MANIFEST_PATH}" "${UPDATES_JSON_PATH}")" ]]; then
    git commit -m "chore(release): bump tb-active-receiver to ${TAG}"
  else
